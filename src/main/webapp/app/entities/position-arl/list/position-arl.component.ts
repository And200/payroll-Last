...
export class PositionArlComponent implements OnInit {
  ...
  protected onSuccess(data: IPositionArl[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/position-arl'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
