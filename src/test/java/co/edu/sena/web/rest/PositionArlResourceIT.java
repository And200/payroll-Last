...
class PositionArlResourceIT {
    ...
    private static final Integer UPDATED_RISK_LEVEL = 2;
    ...
    void createPositionArl() throws Exception {
        ...
        PositionArlDTO positionArlDTO = positionArlMapper.toDto(positionArl);
        restPositionArlMockMvc
            .perform(
                post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(positionArlDTO))
            )
        ...
        assertThat(testPositionArl.getRiskLevel()).isEqualTo(DEFAULT_RISK_LEVEL);
    ...
    void putNewPositionArl() throws Exception {
        ...
        updatedPositionArl.riskLevel(UPDATED_RISK_LEVEL).positionCode(UPDATED_POSITION_CODE).position(UPDATED_POSITION);
        PositionArlDTO positionArlDTO = positionArlMapper.toDto(updatedPositionArl);

        restPositionArlMockMvc
            .perform(
                put(ENTITY_API_URL_ID, positionArlDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(positionArlDTO))
            )
            .andExpect(status().isOk());
        ...
        PositionArl testPositionArl = positionArlList.get(positionArlList.size() - 1);
        assertThat(testPositionArl.getRiskLevel()).isEqualTo(UPDATED_RISK_LEVEL);
    ...
    void putWithMissingIdPathParamPositionArl() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restPositionArlMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(positionArlDTO)))
    ...
    void fullUpdatePositionArlWithPatch() throws Exception {
        ...
        partialUpdatedPositionArl.riskLevel(UPDATED_RISK_LEVEL).positionCode(UPDATED_POSITION_CODE).position(UPDATED_POSITION);
    ...
    void patchNonExistingPositionArl() throws Exception {
        ...
        restPositionArlMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, positionArlDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(positionArlDTO))
            )
            .andExpect(status().isBadRequest());
    ...
    void patchWithMissingIdPathParamPositionArl() throws Exception {
        ...
        restPositionArlMockMvc
            .perform(
                patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(positionArlDTO))
            )
            .andExpect(status().isMethodNotAllowed());
